...
// Starter is a synchronization point for multiple Tickers in goroutines that
// each have some setup but need to start at almost the exaact same time instant.
// In the parent, Add(1) for each goroutine, then Wait() and Broadcast(time.Now()).
