...
<template>
  ...
  <div v-if="providers !== undefined">
    <p>Broker has {{ providers }} connected provider{{ providers > 1 ? "s" : "" }}.</p>
    <p>Current throughput: {{ throughput.toFixed(1) }} tasks/sec</p>
  </div>
  <p v-else>You're not connected to any Broker.</p>
